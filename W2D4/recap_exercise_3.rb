...
def longest_streak(str)
    ...
    streak.split(" ").reverse.max_by { |ele| ele.length }
end
...
def bi_prime?(num)
    ...
    factors.each do |factor1|
        factors.each do |factor2|
            if (factor1 * factor2 == num) && is_prime?(factor1) && is_prime?(factor2)
                return true
            end
        end
    end
...
def vigenere_cipher(message, keys)
    ...
    message.split("").map do |char|
        new_index = (alphabet.index(char) + keys[0]) % 26
        keys.rotate!
        alphabet[new_index]
    end.join("")
end
...
def vowel_rotate(str)
    ...
    str.each_char do |char|
        string_vowels << char if vowels.include?(char)
    end

    string_vowels.unshift(string_vowels.pop)

    str.each_char do |char|
        if vowels.include?(char)
            new_str += string_vowels.shift
         else
            new_str += char
         end
    end
...
def prime_factorization(n)
    ...
    [prime] + prime_factorization(n / prime)
end
...
def next_prime(prime_num)
    return prime_num + 1 if is_prime?(prime_num + 1)
