...
def first_index(arr, &prc)
    arr.each_with_index { |ele, i| return i if prc.call(ele) == true }
    nil
end
...
def filter_out!(arr, &prc)
    arr.each_with_index { |ele, i| arr[i] = nil if prc.call(ele) }
    arr.delete(nil)
    arr
end
...
# p multi_map_alt(['pretty', 'cool', 'huh?']) { |s| s + '!'}      # ["pretty!", "cool!", "huh?!"]
# p multi_map_alt(['pretty', 'cool', 'huh?'], 1) { |s| s + '!'}   # ["pretty!", "cool!", "huh?!"]
# p multi_map_alt(['pretty', 'cool', 'huh?'], 3) { |s| s + '!'}   # ["pretty!!!", "cool!!!", "huh?!!!"]
# p multi_map_alt([4, 3, 2, 7], 1) { |num| num * 10 }             # [40, 30, 20, 70]
# p multi_map_alt([4, 3, 2, 7], 2) { |num| num * 10 }             # [400, 300, 200, 700]
# p multi_map_alt([4, 3, 2, 7], 4) { |num| num * 10 }             # [40000, 30000, 20000, 70000]
...
def chain_map(val, arr_procs)
    arr_procs.each { |proc| val = proc.call(val) }
    val
end
...
def proc_suffix(sentence, hash_procs)
    new_sentence = []

    sentence.split(" ").each do |word| 
        append = word
        # p "append_only_once: " + append.object_id.to_s
        # p "word_only_once: " + word.object_id.to_s
        hash_procs.each do |k, v|
            if k.call(word)
                #append.concat(v) is more memory efficient
                append += v
                # p "append: " + append.object_id.to_s
                # p "word: " + word.object_id.to_s
            end
        end
        new_sentence << append
    end
...
# key is the proc, value is the suffix
...
def proctition(arr, &prc) #helper proctition function
    pass = []
    fail = []
    arr.each do |ele|
        if prc.call(ele)
...
def proctition_platinum(arr, *prcs)
    hash = Hash.new{|the_hash, the_key| the_hash[the_key] = [] }
    prcs.each_with_index {|prc, i| hash[i+1]}

    arr.each do |ele|
        prcs.each_with_index do |proc, i|
            if proc.call(ele)
                hash[i + 1] << ele
                break
            end
...
def procipher(sentence, hash_procs)
    arr = []
    sentence.split(" ").each do |word|
        new_word = word
        hash_procs.each do |k,v|
            if k.call(word)
                new_word = v.call(new_word) #v2.call(v1.call(word))
            end
        end
        arr << new_word
    end
...
#     is_yelled => make_question, #what!???
#     contains_a => reverse #???!tawh
# ) # "he dias ???!tahw"
...
def picky_procipher(sentence, hash_procs)
    arr = []
    sentence.split(" ").each do |word|
        new_word = word
        hash_procs.each do |k,v|
