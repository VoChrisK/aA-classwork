...
#If a character is not a key of the hash, 
...
#TODO:  
def uncompress(str)  #'a2b4c1'
    new_str = ""

    str.each_char.with_index do |char, i|
        if i.odd? 
            char.to_i.times{new_str.concat(str[i-1])}
        end 
    end
...
def conjunct_select(arr, *prcs)
    new_arr = []
    arr.each do |el|
        new_arr << el if procs?(prcs, el) #all?
    end
    new_arr
end
...
def convert_pig_latin(sent)
    ...
    results.each_with_index do |word, i|
        new_results << cap(word)
    end

    new_results.join(" ")
end

def new_word(word) #like -> ikelay
    vowels = "aeiou"
    new_word = ""
...
def reverberate(sent)
    vowels = "aeiou"
    new_sen = sent.split(" ").map do |word|
        if word.length <3
            word
        else
...
def repeat(word)
    ...
    word.each_char do |char|
        if vowels.include?(char)
            new_word.concat(char + letters.join(""))
            break
        end
...
# The method should return a new sentence where the words alternate between having 
...
def alternating_vowel(sent)
    ...
    words.each_with_index do |word, i|
        if i.even?
            new_sent << remove_first_vowel(word)
        else
            new_sent << remove_last_vowel(word)
        end
    end
    new_sent.join(" ")
end
